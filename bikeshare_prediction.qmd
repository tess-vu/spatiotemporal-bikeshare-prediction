...
model4_all <- model.matrix(trips ~ hour + dow_simple + temp + precip + lag_1hr + 
                     lag_3hr + lag_1day + med_inc + pct_public_commute + 
                     pct_white + as.factor(start_station_id),
                   data = train_clean)

fit <- cv.glmnet(X, train_clean$trips, family="gaussian")
```
